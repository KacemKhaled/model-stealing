...
def train_step(model, train_loader, criterion, optimizer, epoch, device, log_interval=10, writer=None):
    ...
    wandb.log({'train_acc_step': acc / 100})

    return train_loss_batch, acc


def test_step(model, test_loader, criterion, device, epoch=0., silent=False, writer=None, mode='test'):
    model.eval()
    test_loss = 0.
    correct = 0
    total = 0
    ...
    if not silent:
        print('[{}]  Epoch: {}\tLoss: {:.6f}\tAcc: {:.1f}% ({}/{})'.format(mode, epoch, test_loss, acc,
                                                                           correct, total))

    if writer is not None:
        writer.add_scalar(f'Loss/{mode}', test_loss, epoch)
        writer.add_scalar(f'Accuracy/{mode}', acc, epoch)

    wandb.log({f'{mode}_loss': test_loss, 'epoch': int(epoch)})
    wandb.log({f'{mode}_acc': acc / 100, 'epoch': int(epoch)})
    wandb.log({'epoch': int(epoch), 'epoch': int(epoch)})
...
def train_model(model, trainset, out_path, batch_size=64, criterion_train=None, criterion_test=None, testset=None,
    ...
    try:
        model_name = f"{kwargs['dataset']}-{kwargs['model_arch']}-{epochs}"
        model_id = f"{kwargs['dataset']}-{kwargs['model_arch']}"
    except:
        # in case of training an adv knockoff testdataset
        model_name = f"{kwargs['testdataset']}-{kwargs['model_arch']}-{epochs}-knockoff{checkpoint_suffix}"
        model_id = f"{kwargs['testdataset']}-{kwargs['model_arch']}-knockoff{checkpoint_suffix}"
    run = wandb.init(project=cfg.WB_PROJECT, entity=cfg.WB_ENTITY, name=model_name,
                     # id=model_name,
                     reinit=True)  # , id=model_name)
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        if test_loader is not None:
            test_loss, test_acc = test_step(model, test_loader, criterion_test, device, epoch=epoch, mode='val')
        ...
        wandb.log({'train_loss_epoch': test_loss, 'epoch': epoch})
        wandb.log({'train_acc_epoch': train_acc / 100, 'epoch': epoch})
        wandb.log({'epoch': int(epoch), 'epoch': epoch})
    ...
    if test_loader is not None:
        test_loss, test_acc = test_step(model, test_loader, criterion_test, device, epoch=epoch, mode='test')
        best_test_acc = max(best_test_acc, test_acc)
    return model
